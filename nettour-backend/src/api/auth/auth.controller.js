...
exports.authEmail = async ctx => {
  ...
  const code = await Account.findByEmail(email);
  ...
  const mailOptions = {
    ...
    html: `<a href=${clientUrl}><img src=${clientUrl}/nettour_logo.png" style="display: block; 
    ...
    회원가입을 위해 이메일 인증을 진행해주세요.
          ...
          <a href="${clientUrl}/auth-email?email=${email}&code=${code.auth.code}" 
...
exports.resetcode = async ctx => {
  ...
  const reset = await Account.findOneAndUpdate(
    { email },
    {
      $set: {
        "auth.code": shortid.generate(),
        "auth.code_created": Date.now(),
      },
    },
    { new: true }
  );
