...
Post.statics.writepost = function ({
  title,
  username,
  content,
  state,
...
Post.statics.list = function ({ cursor, username, user }) {
  ...
  return this.find(query)
    ...
    .limit(20)
    .select(
      "_id thumbnail createdAt username title content likesCount comments url_slug"
    )
    .exec();
};
...
Post.statics.readpost = function ({ name, urlslug, self }) {
  return this.findOne(
    { username: name, url_slug: urlslug },
    { likes: { $elemMatch: { $eq: self } } }
  )
