...
class EditorContainer extends Component {
  ...
  uploadImageCallBack = file => {
    const clientUrl =
      process.env.NODE_ENV === "dev"
        ? process.env.REACT_APP_DEV_CLIENTURL
        : process.env.REACT_APP_PROD_CLIENTURL;
    return new Promise((resolve, reject) => {
      ...
      reader.onload = e => {
        ...
        axios
          .post(`${clientUrl}/api/posts/images`, data)
          .then(function (res) {
            resolve({
              data: {
                link: `${process.env.UPLOAD_URL}/${res.data}`,
              },
            });
          })
          .catch(function (err) {
  ...
  uploadThumbnail = async file => {
    ...
    if (!file || file.size > 1024 * 1024 * 80 || file.type.indexOf("gif") > 0)
      return;
    ...
    return (
      <Fragment>
        <EditorHeader>
          ...
          <Thumbnail onThumbnailupload={handleThumbnail} />
